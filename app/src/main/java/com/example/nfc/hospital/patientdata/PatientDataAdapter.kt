...
class PatientDataAdapter(
    private val registeredUsersList: ArrayList<patientwrapper>,
    private val context: Context
) : RecyclerView.Adapter<PatientDataAdapter.MyViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.patient_card, parent, false);
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.profileDetailsButton.setOnClickListener {
            val intent = Intent(context, record_perosnal::class.java).putExtra(
                "uid",
        ...
        holder.healthRecordButton.setOnClickListener {
            val intent =
                Intent(context, record_medical::class.java).putExtra("uid", user.USER_ID.toString())
            context.startActivity(intent)
        }

        holder.medicalReportButton.setOnClickListener {
            val intent =
                Intent(context, MedicalReport::class.java).putExtra("uid", user.USER_ID.toString())
            context.startActivity(intent)
        }
